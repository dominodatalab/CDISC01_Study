...
%macro run_domino_code;
  ...
  /* Non-workflow (interactive/batch inside Domino workspace) */
  %if &domino_is_workflow_job = false %then %do;
    ...
    *********;
    ...
    /* WPS cannot infer program name automatically here — set explicitly */
    %let __prog_name = t_ae_rel;
    %let __prog_ext  = sas;

    /* If METADATA.&__prog_name. is not present, set sane defaults so the PDF still renders */
    %if %sysfunc(exist(METADATA.&__prog_name.)) = 0 %then %do;
      %let DisplayName  = CDISC01;
      %let DisplayTitle = Treatment-Emergent Adverse Events by Relatedness;
      %let Title1       = Safety Population;
      %let Footer1      = Source: ADaM.ADSL / ADAE;
      %let Footer2      = Program: &__prog_name..sas;
      %let Footer3      = Run date: %sysfunc(date(),worddate.);
      %let Footer4      = ;
      %let Footer5      = ;
    %end;
    %else %do;
      /* Use standard project metadata if present */
      %tfl_metadata;
    %end;

    /* Style: inherit from a printer style to avoid WPS "parent fonts" error */
    ...
    /* Build analysis datasets with consistent key lengths to avoid BY-length warnings */
    ...
    ** exclude non-treated subjects;
    ...
    %macro ne_freq (indat =, outdat =, byvars =, anyvars = );
      ...
      %if &byvars ^= %then %let sep = %str( , ); %else %let sep = ;
      proc sql;
    ...
    /* ADSL with total column for denominators */
    data adslmod;
      set &inadsl
          &inadsl (in=intot);
      if intot then &trtvarn = &totval;
    ...
    /* Counts including totals (by Relatedness) */
    ...
    /* Counts including totals (Any Relatedness) */
    ...
    /* Template dataset, then force lengths to match before merging */
    proc sql;
      create table template_raw as
    ...
    proc rank data=counts_w0 (where=(&trtvarn=&trtnord & &ptvar='ANY' & &socvar^='ANY' & &byvar='ANY'))
              out=socranks (keep=&socvar SOCord_NP SOCord_EV)
              ties=low descending;
      var stat_n stat_e;
    ...
    proc rank data=counts_w0s (where=(&trtvarn=&trtnord & &ptvar^='ANY' & &socvar^='ANY' & &byvar='ANY'))
              out=ptranks (keep=&socvar &ptvar PTord_NP PTord_EV)
              ties=low descending;
      by &socvar;
      var stat_n stat_e;
    ...
    proc format; invalue relord 'ANY'=1 'Related'=2 'Not Related'=3; run;
    ...
    proc sql;
      ...
      from counts_cat a
      left join socranks b on a.&socvar=b.&socvar
      left join ptranks c  on a.&socvar=c.&socvar and a.&ptvar=c.&ptvar
    ...
    data counts_T; set counts_T; where &trtvarn ^= .; run;

    /* Transpose again so treatments are columns */
    ...
    data extrow (keep=aesoc aedecod soc_pt_disp &byvar trt_99_npp);
      ...
      if first.aesoc & aesoc^='ANY' then do; soc_pt_disp=aesoc; output; end;
      if aesoc='ANY' then do; soc_pt_disp='Subjects with at least one TEAE'; output; end;
    ...
    data final (drop=i);
      ...
      if aedecod = soc_pt_disp then indent='Y'; else indent='N';
    ...
    proc report data=final headline split="*" style(report)={width=100%};
      ...
      define &byvar      / display '' style(header)={width=8%  just=l};
      ...
      define trt_99_npp  / display "All Treatments*(N=&N_99)*n (%)"
                           style(header)={width=30%} style(column)={just=r};  /* was just=d */
  ...
  /* Workflow/Flows branch */
  %else %if &domino_is_workflow_job = true %then %do;

    ** Setup environment including libraries for this reporting effort;

    * ==================================================================;
    * Set SASAUTOS to search for shared macros (usually in domino.sas) ;
    * ==================================================================;
    ...
    /* Macro and analysis steps identical to non-workflow branch */
    %macro ne_freq (indat =, outdat =, byvars =, anyvars = );
      %let byvars2  = %sysfunc(tranwrd(%quote(&byvars.),%str( ),%str( , )));
      %if &byvars ^= %then %let sep = %str( , ); %else %let sep = ;
      proc sql;
    ...
    data aes;
      set &indat &indat(in=intot);
      if intot then &trtvarn = &totval;
    run;

    data adslmod;
      set &inadsl &inadsl(in=intot);
      if intot then &trtvarn = &totval;
    run;
    ...
    data allcounts; length &socvar &ptvar $200; set out1-out6; run;
    ...
    proc sql;
      ...
      ;
    quit;
    data template; length &socvar &ptvar $200; set template_raw; run;

    proc sort data=allcounts out=allcounts_s; by &trtvarn &socvar &ptvar &byvar; run;
    ...
    proc rank data=counts_w0 (where=(&trtvarn=&trtnord & &ptvar='ANY' & &socvar^='ANY' & &byvar='ANY'))
              out=socranks (keep=&socvar SOCord_NP SOCord_EV)
              ties=low descending;
      var stat_n stat_e; ranks SOCord_NP SOCord_EV;
    run;

    proc sort data=counts_w0 out=counts_w0s; by &socvar; run;

    proc rank data=counts_w0s (where=(&trtvarn=&trtnord & &ptvar^='ANY' & &socvar^='ANY' & &byvar='ANY'))
              out=ptranks (keep=&socvar &ptvar PTord_NP PTord_EV)
              ties=low descending;
      by &socvar; var stat_n stat_e; ranks PTord_NP PTord_EV;
    run;
    ...
    /* Pull titles/footers from METADATA.t_ae_rel */
    data metadata; set metadata.t_ae_rel; run;
    data _null_;
      set metadata;
      array xxx {*} _numeric_;
      do i=1 to dim(xxx); call symput(vname(xxx[i]), xxx[i]); end;
      array yyy {*} $ _character_;
      do i=1 to dim(yyy); call symput(vname(yyy[i]), yyy[i]); end;
    run;

    ods pdf file="/workflow/outputs/t_ae_rel.pdf" style=newstyle;
    ods noproctitle;
    ods escapechar="^";
    ...
    proc report data=final headline split="*" style(report)={width=100%} out = outputs.t_ae_rel_data;
      ...
      compute before aedecod; line ''; endcomp;
