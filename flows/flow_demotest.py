...
# pyflyte run --remote --name test-run flow_demotest.py ADaM_TFL_test --sdtm_dataset_snapshot /mnt/imported/data/SDTMBLIND --metadata_snapshot /mnt/data/METADATA 

DataArtifact = Artifact("ADaM Datasets", DATA)
ReportArtifact = Artifact("TFL Reports", REPORT)

@workflow
def ADaM_TFL_test(sdtm_dataset_snapshot: str, metadata_snapshot: str): 

   #Crete ADSL dataset. The only input is the SDTM Dataset. 
    adsl_task = run_domino_job_task(
        flyte_task_name="Create ADSL Dataset new name",
